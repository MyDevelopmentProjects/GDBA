...
import ge.unknown.service.GalleryCategoryService;
 ...
 */
@Controller
@RequestMapping("/gallery-category")
public class GalleryCategoryController {
    ...
    public PaginationAndFullSearchQueryResult getList(
        ...
        return galleryCategoryService.getList(searchExpression, sortField, isAscending, pageNumber, pageSize, category);
    }

    @RequestMapping(value = SLASH + PUT, method = RequestMethod.POST)
    @ResponseBody
    public RequestResponse save(@RequestBody GalleryCategory category) {
        galleryCategoryService.save(category);
    ...
    public RequestResponse delete(@RequestBody Long id) {
        boolean deleted = galleryCategoryService.delete(id);
        if (deleted) {
            return GeneralUtil.RequestSuccess();
        }
