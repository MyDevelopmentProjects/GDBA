...
import ge.unknown.service.WhyChooseUsService;
...
public class WhyUsController {
    ...
    public PaginationAndFullSearchQueryResult getList(
        ...
        return whyChooseUsService.getList(searchExpression, sortField, isAscending, pageNumber, pageSize);
    }


    @RequestMapping(value = SLASH + PUT, method = RequestMethod.POST)
    @ResponseBody
    public RequestResponse save(@RequestBody WhyChooseUs whyChooseUs) {
        whyChooseUsService.save(whyChooseUs);
    ...
    public RequestResponse delete(@RequestBody Long id) {
        boolean deleted = whyChooseUsService.delete(id);
        if (deleted) {
            return GeneralUtil.RequestSuccess();
        }
